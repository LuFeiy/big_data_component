...
public class BatchWordCount {
    public static void main(String[] args) throws Exception {
        System.out.println("BatchWordCount");
        // 1. 创建执行环境
        ExecutionEnvironment env = ExecutionEnvironment.getExecutionEnvironment();
        // 2. 从文件读取数据 按行读取(存储的元素就是每行的文本)
        DataSource<String> lineDS = env.readTextFile("hdfs://hadoop102:8020/tmp/input/wcinput");
        // 3. 转换数据格式
        ...
        // 6. 输出结果
        //sum.print();
        sum.writeAsCsv("hdfs://hadoop102:8020/tmp/output/res.csv", "\n", ",");
