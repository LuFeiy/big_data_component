...
public class TS_05_StreamOperate {

    //分流
    @Test
    public void test_01_diversion() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();

        SingleOutputStreamOperator<Integer> ds = env.socketTextStream("172.16.50.205", 7777)
                .map(Integer::valueOf);
    ...
    //测流输出
    @Test
    public void test_02_sideStream() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();

        SingleOutputStreamOperator<String> ds = env.socketTextStream("172.16.50.205", 7777);

        OutputTag<String> hiSideOutputTag = new OutputTag<String>("hiSide") {};
        OutputTag<String> byeSideOutputTag = new OutputTag<String>("byeSide") {};

        //返回的就是主流
        SingleOutputStreamOperator<String> dsMain = ds.process(new ProcessFunction<String, String>() {
            @Override
            public void processElement(String s, ProcessFunction<String, String>.Context context, Collector<String> collector) throws Exception {
                if (s.startsWith("hi")){
                    context.output(hiSideOutputTag,s);
                } else if (s.startsWith("bye")) {
                    context.output(byeSideOutputTag,s);
                } else {
                    collector.collect(s);
                }
            }
        ...
        SideOutputDataStream<String> hiDs = dsMain.getSideOutput(hiSideOutputTag);
        hiDs.printToErr("hiDs");

        SideOutputDataStream<String> byeDs = dsMain.getSideOutput(byeSideOutputTag);
        byeDs.printToErr("byeDs");

        env.execute();

    }

    //union:要求元素类型必须一致，可以多条流
    @Test
    public void test_03_union() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
     ...
     */
    @Test
    public void test_04_connect() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();

        env.setParallelism(1);

        DataStreamSource<Integer> ds1 = env.fromElements(2, 2, 3);
        DataStreamSource<String> ds2 = env.fromElements("32", "22", "13");
    ...
    @Test
    public void test_05_connect_InnerJoin() throws Exception {
        StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
        env.setParallelism(2);
